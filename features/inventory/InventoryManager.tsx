    ...
    return (
        <>
            {activeTab === 'equipment.inventory' && (
                <EquipmentDashboard equipment={equipmentData} totalItems={totalEquipment} loading={equipmentLoading} page={equipmentPage} pageSize={equipmentPageSize} sort={equipmentSort} onPageChange={setEquipmentPage} onPageSizeChange={setEquipmentPageSize} onSortChange={setEquipmentSort} onFilterChange={setDashboardFilter} brands={appData.brands} equipmentTypes={appData.equipmentTypes} brandMap={new Map(appData.brands.map((b: Brand) => [b.id, b.name]))} equipmentTypeMap={new Map(appData.equipmentTypes.map((t: EquipmentType) => [t.id, t.name]))} assignedEquipmentIds={new Set(appData.assignments.filter((a: Assignment) => !a.return_date).map((a: Assignment) => a.equipment_id))} assignments={appData.assignments} collaborators={appData.collaborators} entidades={appData.entidades} instituicoes={appData.instituicoes} initialFilter={dashboardFilter} onClearInitialFilter={() => setDashboardFilter(null)} onAssign={checkPermission('equipment', 'edit') ? (eq: Equipment) => { setEquipmentToAssign(eq); setShowAssignModal(true); } : undefined} onUnassign={checkPermission('equipment', 'edit') ? async (id: string) => { await dataService.addAssignment({ equipment_id: id, return_date: new Date().toISOString().split('T')[0] }); fetchEquipment(); } : undefined} onUpdateStatus={checkPermission('equipment', 'edit') ? async (id: string, status: EquipmentStatus) => { await dataService.updateEquipment(id, { status }); fetchEquipment(); } : undefined} onShowHistory={(eq: Equipment) => { setDetailEquipment(eq); }} onEdit={checkPermission('equipment', 'edit') ? (eq: Equipment) => { setEquipmentToEdit(eq); setShowAddEquipmentModal(true); } : undefined} onDelete={checkPermission('equipment', 'delete') ? async (id: string) => { await dataService.deleteEquipment(id); fetchEquipment(); refreshGlobalData(); } : undefined} onCreate={checkPermission('equipment', 'create') ? () => { setEquipmentToEdit(null); setShowAddEquipmentModal(true); } : undefined} businessServices={appData.businessServices} serviceDependencies={appData.serviceDependencies} tickets={appData.tickets} ticketActivities={appData.ticketActivities} softwareLicenses={appData.softwareLicenses} licenseAssignments={appData.licenseAssignments} vulnerabilities={appData.vulnerabilities} suppliers={appData.suppliers} procurementRequests={appData.procurementRequests} tooltipConfig={userTooltipConfig} onViewItem={onViewItem} accountingCategories={appData.configAccountingCategories} conservationStates={appData.configConservationStates} />
            )}
            ...
            {showAddEquipmentModal && <AddEquipmentModal onClose={() => setShowAddEquipmentModal(false)} onSave={handleSaveEquipment} brands={appData.brands} equipmentTypes={appData.equipmentTypes} equipmentToEdit={equipmentToEdit} onSaveBrand={dataService.addBrand} onSaveEquipmentType={dataService.addEquipmentType} onOpenKitModal={()=>{}} suppliers={appData.suppliers} softwareLicenses={appData.softwareLicenses} entidades={appData.entidades} collaborators={appData.collaborators} statusOptions={appData.configEquipmentStatuses} licenseAssignments={appData.licenseAssignments} onOpenHistory={(eq: Equipment) => setDetailEquipment(eq)} onManageLicenses={(eq: Equipment) => setDetailEquipment(eq)} onOpenAssign={(eq: Equipment) => { setEquipmentToAssign(eq); setShowAssignModal(true); }} accountingCategories={appData.configAccountingCategories} conservationStates={appData.configConservationStates} cpuOptions={appData.configCpus} ramOptions={appData.configRamSizes} storageOptions={appData.configStorageTypes} />}
