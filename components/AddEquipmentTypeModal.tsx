...
const AddEquipmentTypeModal: React.FC<AddEquipmentTypeModalProps> = ({ onClose, onSave, typeToEdit, existingTypes = [], teams = [] }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Ensure empty strings are handled for UUID fields
        const dataToSave = {
            ...formData,
            name,
        ...
        <Modal title={modalTitle} onClose={onClose}>
            <form onSubmit={handleSubmit} className="space-y-4 max-h-[75vh] overflow-y-auto pr-2 custom-scrollbar">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label htmlFor="default_team_id" className="block text-sm font-bold text-gray-400 mb-1 uppercase tracking-widest text-[10px]">Equipa Padrão (Auto-atribuição)</label>
                        <select 
                    ...
                    <h3 className="text-[10px] font-black text-brand-secondary uppercase tracking-widest mb-4">Campos Requeridos no Inventário</h3>
                     <div className="space-y-3 grid grid-cols-1 sm:grid-cols-2 gap-x-6">
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_nome_na_rede" checked={formData.requires_nome_na_rede} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">Nome na Rede</span>
                        </label>
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_ip" checked={formData.requires_ip} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">Endereço IP</span>
                        </label>
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_mac_wifi" checked={formData.requires_mac_wifi} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">MAC WiFi</span>
                        </label>
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_mac_cabo" checked={formData.requires_mac_cabo} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">MAC Cabo (Ethernet)</span>
                        </label>
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_inventory_number" checked={formData.requires_inventory_number} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">Nº de Património / Etiqueta</span>
                        </label>
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_location" checked={formData.requires_location} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">Localização Física</span>
                        </label>
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_ram_size" checked={formData.requires_ram_size} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">Memória RAM</span>
                        </label>
                         <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_disk_info" checked={formData.requires_disk_info} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">Armazenamento / Discos</span>
                        </label>
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_cpu_info" checked={formData.requires_cpu_info} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">Processador (CPU)</span>
                        </label>
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_wwan_address" checked={formData.requires_wwan_address} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">Endereço IMEI/WWAN</span>
                        </label>
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_bluetooth_address" checked={formData.requires_bluetooth_address} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">Endereço Bluetooth</span>
                        </label>
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_usb_thunderbolt_address" checked={formData.requires_usb_thunderbolt_address} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                            <span className="ml-2 text-sm text-gray-300 group-hover:text-white transition-colors">Endereço USB/TB</span>
                        </label>
                        <label className="flex items-center cursor-pointer group">
                            <input type="checkbox" name="requires_manufacture_date" checked={formData.requires_manufacture_date} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                        ...
                        <label className="flex items-center cursor-pointer group sm:col-span-2 bg-indigo-900/10 p-2 rounded border border-indigo-500/20">
                            <input type="checkbox" name="requires_backup_test" checked={formData.requires_backup_test} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
                        ...
                        <label className="flex items-center cursor-pointer group sm:col-span-2 bg-orange-900/10 p-2 rounded border border-orange-500/20">
                            <input type="checkbox" name="is_maintenance" checked={formData.is_maintenance} onChange={handleChange} className="h-4 w-4 rounded border border-gray-500 bg-gray-700 text-brand-primary focus:ring-brand-secondary" />
