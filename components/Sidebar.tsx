...
const Sidebar: React.FC<SidebarProps> = ({ currentUser, activeTab, setActiveTab, onLogout, tabConfig, notificationCount, onNotificationClick, isExpanded, onHover, onOpenProfile, onOpenCalendar, onOpenManual }) => {
    ...
    // Fix: Renamed state variables to match usage in JSX and avoid 'Cannot find name' errors.
    ...
    const hasOverviewTabs = tabConfig['overview'] || tabConfig['overview.smart'] || tabConfig['my_area'];
    ...
    const TabButton = ({ tab, label, icon, activeTab, setActiveTab, isDropdownItem = false, className = '', onClick }: { tab?: string, label: string, icon: React.ReactNode, activeTab?: string, setActiveTab?: (tab: string) => void, isDropdownItem?: boolean, className?: string, onClick?: () => void }) => {
        const handleClick = (e: React.MouseEvent) => {
            ...
            if (window.innerWidth < 768) onHover(false); 
        };
    ...
    return (
        <>
        <div className={`fixed inset-0 bg-black/60 z-40 transition-opacity duration-300 md:hidden ${isExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => onHover(false)} />

        <aside 
            className={`z-50 flex flex-col border-r border-gray-800 bg-gray-900 shadow-2xl transition-all duration-300 ease-in-out fixed top-0 left-0 h-screen md:relative md:h-auto ${isExpanded ? 'translate-x-0 w-72 md:w-64' : '-translate-x-full md:translate-x-0 md:w-20'}`}
        ...
        >
            <div className="flex items-center justify-between h-20 flex-shrink-0 bg-gray-900 border-b border-gray-800 px-4">
                ...
                <button className="md:hidden text-gray-400 p-2 rounded hover:bg-gray-800" onClick={() => onHover(false)}><FaTimes className="h-6 w-6" /></button>
            </div>

            <nav className="flex-grow py-4 px-2 space-y-1 overflow-y-auto custom-scrollbar overflow-x-hidden">
                {hasOverviewTabs && (
                    <div className="space-y-1">
                        <button onClick={() => setIsOverviewOpen(!isOverviewOpen)} className={`flex items-center justify-between w-full px-4 py-3 text-base md:text-sm font-medium rounded-md transition-colors duration-200 ${isOverviewOpen ? 'bg-gray-800 text-white' : 'text-on-surface-dark-secondary hover:bg-gray-800'}`}>
                            ...
                            {isExpanded && <span className={`transition-transform duration-200 ${isOverviewOpen ? 'rotate-90' : ''}`}><FaChevronRight className="w-3 h-3" /></span>}
                        </button>
                ...
                {hasOrganizacaoTabs && (
                    <div className="space-y-1">
                        <button onClick={() => setOrganizationOpen(!isOrganizationOpen)} className={`flex items-center justify-between w-full px-4 py-3 text-base md:text-sm font-medium rounded-md transition-colors duration-200 ${isOrganizationOpen ? 'bg-gray-800 text-white' : 'text-on-surface-dark-secondary hover:bg-surface-dark hover:text-white'}`}>
                            <div className="flex items-center gap-3 overflow-hidden whitespace-nowrap">
                            ...
                            {isExpanded && <span className={`transition-transform duration-200 ${isOrganizationOpen ? 'rotate-90' : ''}`}><FaChevronRight className="w-3 h-3" /></span>}
                        </button>
                ...
                {hasInventarioTabs && (
                    <div className="space-y-1">
                        <button onClick={() => setInventoryOpen(!isInventoryOpen)} className={`flex items-center justify-between w-full px-4 py-3 text-base md:text-sm font-medium rounded-md transition-colors duration-200 ${isInventoryOpen ? 'bg-gray-800 text-white' : 'text-on-surface-dark-secondary hover:bg-surface-dark hover:text-white'}`}>
                            <div className="flex items-center gap-3 overflow-hidden whitespace-nowrap">
                            ...
                            {isExpanded && <span className={`transition-transform duration-200 ${isInventoryOpen ? 'rotate-90' : ''}`}><FaChevronRight className="w-3 h-3" /></span>}
                        </button>
                ...
                {hasNis2Tabs && (
                    <div className="space-y-1">
                        <button onClick={() => setIsNis2Open(prev => !prev)} className={`flex items-center justify-between w-full px-4 py-3 text-base md:text-sm font-medium rounded-md transition-colors duration-200 ${isNis2Open ? 'bg-gray-800 text-white' : 'text-on-surface-dark-secondary hover:bg-surface-dark hover:text-white'}`}>
                            <div className="flex items-center gap-3 overflow-hidden whitespace-nowrap">
                            ...
                            {isExpanded && <span className={`transition-transform duration-200 ${isNis2Open ? 'rotate-90' : ''}`}><FaChevronRight className="w-3 h-3" /></span>}
                        </button>
                ...
                {hasTicketTabs && <TabButton tab="tickets.list" label={t('nav.tickets')} icon={<FaTicketAlt />} activeTab={activeTab} setActiveTab={setActiveTab}/>}
                {hasReportsTabs && <TabButton tab="reports" label={t('nav.reports')} icon={<FaFileSignature />} activeTab={activeTab} setActiveTab={setActiveTab}/>}

                {hasToolsTabs && (
                    <div className="space-y-1">
                        <button onClick={() => setIsToolsOpen(!isToolsOpen)} className={`flex items-center justify-between w-full px-4 py-3 text-base md:text-sm font-medium rounded-md transition-colors duration-200 ${isToolsOpen ? 'bg-gray-800 text-white' : 'text-on-surface-dark-secondary hover:bg-surface-dark hover:text-white'}`}>
                            <div className="flex items-center gap-3 overflow-hidden whitespace-nowrap">
                            ...
                            {isExpanded && <span className={`transition-transform duration-200 ${isToolsOpen ? 'rotate-90' : ''}`}><FaChevronRight className="w-3 h-3" /></span>}
                        </button>
            ...
            <div className="p-4 border-t border-gray-800 bg-gray-900 flex-shrink-0 relative">
                {currentUser && (
                    <div className="relative">
                        ...
                        {isUserMenuOpen && (
                            <div className="absolute bottom-full left-0 w-64 mb-2 bg-surface-dark border border-gray-700 rounded-md shadow-xl py-1 z-[100] overflow-hidden">
                                ...
                                <div className="py-1">
                                    ...
                                    <button onClick={() => { setLayoutMode('top'); setIsUserMenuOpen(false); }} className="flex w-full items-center gap-3 px-4 py-2 text-sm text-on-surface-dark hover:bg-gray-700">
                                        <FaColumns className="text-gray-400" /> Menu Superior
                                    </button>
                                    <button onClick={() => { setShowMFA(true); setIsUserMenuOpen(false); }} className="flex w-full items-center gap-3 px-4 py-2 text-sm text-on-surface-dark hover:bg-gray-700">
                                        <FaFingerprint className="text-brand-secondary" /> Configurar 2FA
                                    </button>
                                    <div className="px-4 py-2">
                                        <InstallAppButton className="flex w-full items-center justify-center gap-2 px-2 py-2 text-xs font-bold text-gray-900 bg-brand-secondary rounded-md" label="Instalar App" icon={<FaMobileAlt />} />
                                    </div>
                                    {isAdmin && (
                                        <>
                                            <button onClick={() => { setActiveTab('settings'); setIsUserMenuOpen(false); }} className="flex w-full items-center gap-3 px-4 py-2 text-sm text-on-surface-dark hover:bg-gray-700">
                                                <FaCog className="text-brand-secondary" /> {t('common.settings')}
                                            </button>
                                            <button onClick={() => { setShowAudit(true); setIsUserMenuOpen(false); }} className="flex w-full items-center gap-3 px-4 py-2 text-sm text-on-surface-dark hover:bg-gray-700">
                                                <FaClipboardList className="text-yellow-400" /> {t('common.audit')}
                                            </button>
                                            {isSuperAdmin && (
                                                <button onClick={() => { setShowDbSchema(true); setIsUserMenuOpen(false); }} className="flex w-full items-center gap-3 px-4 py-2 text-sm text-on-surface-dark hover:bg-gray-700">
                                                    <FaDatabase className="text-green-400" /> {t('common.database')}
                                                </button>
                                            )}
                                        </>
                                    )}
