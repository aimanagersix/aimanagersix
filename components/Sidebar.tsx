                ...
                <nav className="flex-grow py-4 px-2 space-y-1 overflow-y-auto custom-scrollbar overflow-x-hidden">
                    ...
                    <div className="space-y-1">
                        ...
                        {openMenus.org && isExpanded && (
                            <div className="pl-4 space-y-1 bg-gray-800/30 rounded-md py-1 animate-fade-in">
                                <TabButton tab="organizacao.instituicoes" label={t('nav.institutions')} icon={<FaSitemap />} isDropdownItem />
                                <TabButton tab="organizacao.entidades" label={t('nav.entities')} icon={<FaBuilding />} isDropdownItem />
                                <TabButton tab="collaborators" label={t('nav.collaborators')} icon={<FaUsers />} isDropdownItem />
                                <TabButton tab="organizacao.teams" label={t('nav.teams')} icon={<FaUsers />} isDropdownItem />
                                <TabButton tab="organizacao.suppliers" label={t('nav.suppliers')} icon={<FaShieldAlt />} isDropdownItem />
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-1">
                        ...
                        {openMenus.inv && isExpanded && (
                            <div className="pl-4 space-y-1 bg-gray-800/30 rounded-md py-1 animate-fade-in">
                                <TabButton tab="equipment.inventory" label={t('nav.assets_inventory')} icon={<FaClipboardList />} isDropdownItem />
                                <TabButton tab="licensing" label={t('nav.licensing')} icon={<FaKey />} isDropdownItem />
                                <TabButton tab="equipment.procurement" label={t('nav.procurement')} icon={<FaShoppingCart />} isDropdownItem />
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-1">
                        ...
                        {openMenus.compliance && isExpanded && (
                            <div className="pl-4 space-y-1 bg-gray-800/30 rounded-md py-1 animate-fade-in">
                                <TabButton tab="nis2.bia" label={t('nav.bia')} icon={<FaNetworkWired />} isDropdownItem />
                                <TabButton tab="nis2.security" label={t('nav.security')} icon={<FaShieldAlt />} isDropdownItem />
                                <TabButton tab="nis2.backups" label={t('nav.backups')} icon={<FaServer />} isDropdownItem />
                                <TabButton tab="nis2.resilience" label={t('nav.resilience')} icon={<FaShieldVirus className="text-purple-400"/>} isDropdownItem />
                            </div>
                        )}
                    </div>

                    <TabButton tab="tickets.list" label={t('nav.tickets')} icon={<FaTicketAlt />} />
                    <TabButton tab="reports" label={t('nav.reports')} icon={<FaFileSignature />} />
