...
const RegulatoryNotificationModal: React.FC<RegulatoryNotificationModalProps> = ({ ticket, activities, onClose }) => {
    ...
    // Calculate Deadlines (24h and 72h)
        ...
        <Modal title="Notificação Regulatória (NIS2 / DORA)" onClose={onClose} maxWidth="max-w-4xl">
            <div className="space-y-6">
                ...
                <div className="bg-blue-900/20 border border-blue-900/50 p-4 rounded-lg text-sm text-blue-200">
                    ...
                    <p>
                        Incidentes de segurança significativos devem ser notificados à autoridade nacional (CSIRT/CNCS) nos prazos estipulados.
                        Este assistente gera o ficheiro JSON standard e anonimiza dados pessoais.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className={`p-4 rounded-lg border ${status24h.bg} flex justify-between items-center`}>
                        <div>
                            ...
                            <p className="text-xs text-gray-400">Obrigatório para indicar suspeita de ato malicioso.</p>
                        </div>
                        <div className={`text-xl ${status24h.color}`}>
                            {status24h.text}
                        </div>
                    </div>
                    <div className={`p-4 rounded-lg border ${status72h.bg} flex justify-between items-center`}>
                        <div>
                            ...
                            <p className="text-xs text-gray-400">Relatório completo com avaliação inicial.</p>
                        </div>
                        <div className={`text-xl ${status72h.color}`}>
                            {status72h.text}
                        </div>
                    </div>
                </div>
                ...
                {!reportJson && (
                    <div className="flex justify-center py-8">
                        <button 
                            ...
                            className="flex items-center gap-3 px-6 py-3 bg-brand-primary text-white rounded-full hover:bg-brand-secondary transition-transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg font-bold"
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                        <div className="flex flex-col h-96">
                            <div className="flex justify-between items-center mb-2">
                                <h4 className="text-sm font-bold text-gray-300">JSON Output (Standard CSIRT)</h4>
                                <div className="flex gap-2">
                                    <button onClick={handleCopy} className="text-xs bg-gray-700 px-2 py-1 rounded hover:bg-gray-600 text-white flex items-center gap-1">
                                        {copied ? <FaCheck className="text-green-400"/> : <FaCopy />} Copiar
                                    </button>
                                    <button onClick={handleDownload} className="text-xs bg-gray-700 px-2 py-1 rounded hover:bg-gray-600 text-white flex items-center gap-1">
                                        <FaDownload /> Download
                                    </button>
                                </div>
                            </div>
                            <pre className="bg-gray-900 text-green-400 p-4 rounded-lg text-xs font-mono overflow-auto flex-grow custom-scrollbar border border-gray-700">
                                {JSON.stringify(JSON.parse(reportJson), null, 2)}
                            </pre>
                        </div>

                        <div className="flex flex-col h-96">
                            <h4 className="text-sm font-bold text-gray-300 mb-2">Resumo Legível</h4>
                            <div 
                                className="bg-gray-800 p-4 rounded-lg text-sm text-gray-300 overflow-auto flex-grow custom-scrollbar border border-gray-700 prose prose-invert prose-sm max-w-none"
                                dangerouslySetInnerHTML={{ __html: reportSummary }}
                            />
                        </div>
                    </div>
                )}
