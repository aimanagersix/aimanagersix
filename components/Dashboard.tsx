    ...
    <div className="bg-surface-dark p-6 rounded-lg shadow-xl">
            ...
            {paginatedEquipment.length > 0 ? paginatedEquipment.map((item) => {
              ...
              >
                ...
                <td className="px-6 py-4 font-medium text-on-surface-dark whitespace-nowrap">
                  ...
                  <div className="text-xs text-on-surface-dark-secondary">{brandMap.get(item.brandId) || 'N/A'} / {equipmentTypeMap.get(item.typeId) || 'N/A'}</div>
                </td>
                ...
                <td className="px-6 py-4">{item.purchaseDate}</td>
                 <td className={`px-6 py-4 ${warrantyInfo.className}`}>
                  {warrantyInfo.text}
                </td>
                <td className="px-6 py-4">
                    <select
                        ...
                        aria-label={`Status for ${item.serialNumber}`}
                        disabled={!onUpdateStatus}
                    >
                        {Object.values(EquipmentStatus).map(s => 
                            <option key={s} value={s} disabled={isAssigned && s === EquipmentStatus.Stock}>
                                {s}
                            </option>
                        )}
                    </select>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center justify-center gap-4">
                    {onAssign && onUnassign && (() => {
                      const isDecommissioned = item.status === EquipmentStatus.Decommissioned;

                      if (isDecommissioned) {
                        return (
                          <button disabled className="font-medium text-gray-500 cursor-not-allowed flex items-center justify-center gap-1 w-28">
                            <AssignIcon />
                            Atribuir
                          </button>
                        );
                      ...
                      if (isAssigned) {
                        return (
                          <button onClick={() => onUnassign(item.id)} className="font-medium text-yellow-400 hover:text-yellow-300 flex items-center justify-center gap-1 w-28">
                            <UnassignIcon />
                            Desassociar
                          </button>
                        );
                      ...
                      return (
                        <button onClick={() => onAssign(item)} className="font-medium text-brand-secondary hover:underline flex items-center justify-center gap-1 w-28">
                          <AssignIcon />
                          Atribuir
                        </button>
                      );
                    ...
                    {onEdit && (
                        <button onClick={() => onEdit(item)} className="text-blue-400 hover:text-blue-300" aria-label={`Editar ${item.serialNumber}`}>
                            <EditIcon className="h-5 w-5"/>
                        </button>
                    )}
                     <button onClick={() => onManageLicenses(item)} className="text-yellow-400 hover:text-yellow-300" title="Gerir Licenças">
                        <FaKey className="h-4 w-4" />
                    </button>
                     <button onClick={() => onShowHistory(item)} className="text-gray-400 hover:text-white" aria-label={`Histórico de ${item.serialNumber}`}>
                ...
                <tr>
                    <td colSpan={10} className="text-center py-8 text-on-surface-dark-secondary">
      ...
      {tooltip?.visible && (
            <div
                ...
                className="bg-gray-900 text-white text-sm rounded-md shadow-lg p-3 z-50 border border-gray-700 max-w-xs"
                role="tooltip"
            >
                {tooltip.content}
            </div>
