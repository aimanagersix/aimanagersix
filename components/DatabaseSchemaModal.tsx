...
/**
 * DB Manager UI - v29.1 (Dynamic SLA & RLS Fix Patch)
 * -----------------------------------------------------------------------------
...
const DatabaseSchemaModal: React.FC<DatabaseSchemaModalProps> = ({ onClose }) => {
    ...
    const dynamicSlaPatch = `-- 🕒 AIMANAGER - DYNAMIC SLA & RLS REPAIR PATCH (v29.1)
...
-- 2. REPARAÇÃO INTEGRAL RLS: HOLIDAYS (Fix: New row violates RLS)
-- Garante que SuperAdmins e Admins têm bypass total de validação de ID
ALTER TABLE public.holidays ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Holidays_View_Policy" ON public.holidays;
CREATE POLICY "Holidays_View_Policy" ON public.holidays 
FOR SELECT TO authenticated 
USING (true);

DROP POLICY IF EXISTS "Holidays_Manage_Policy" ON public.holidays;
CREATE POLICY "Holidays_Manage_Policy" ON public.holidays 
                ...
                <div className="flex-grow overflow-hidden flex flex-col gap-4">
                    ...
                    {activeTab === 'dynamic_sla_patch' && (
                        <div className="bg-yellow-900/20 border border-yellow-500/30 p-4 rounded-lg mb-2">
                            <h4 className="text-yellow-400 font-bold flex items-center gap-2 text-sm uppercase mb-2"><FaClock /> FIX: VIOLAÇÃO RLS EM FERIADOS</h4>
                            <p className="text-[11px] text-gray-300">Este patch corrige o erro que impedia SuperAdmins de criar feriados globais (com collaborator_id nulo) e habilita o SLA Dinâmico.</p>
                        </div>
                    )}
