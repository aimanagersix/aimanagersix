...
const AddSupplierModal: React.FC<AddSupplierModalProps> = ({ onClose, onSave, supplierToEdit }) => {
    ...
    const handleFetchVies = async () => {
        ...
        try {
            ...
            // Se tiver letras no início, usa como country code
            if (/^[A-Z]{2}/.test(input)) {
                countryCode = input.substring(0, 2);
                vatNumber = input.substring(2);
            }

            // Chamada à API REST do VIES
            const response = await fetch(`https://ec.europa.eu/taxation_customs/vies/rest-api/check-vat-number/${countryCode}/${vatNumber}`);
            ...
            if (data.isValid) {
                setFormData(prev => ({
                    ...
                    nif: input, // Normaliza o input
                    // Adiciona o endereço às notas se existir
            ...
            alert("Não foi possível obter dados do VIES automaticamente. Por favor, preencha os dados manualmente.");
        } finally {
                    ...
                    <div>
                        ...
                        <div className="flex">
                            ...
                            <button 
                                ...
                                title="Pesquisar no VIES"
                            >
                                {isFetchingVies ? <SpinnerIcon /> : <SearchIcon />}
                            </button>
                        </div>
                        {errors.nif && <p className="text-red-400 text-xs italic mt-1">{errors.nif}</p>}
                        <p className="text-[10px] text-gray-500 mt-1">Use a lupa para preencher automaticamente via VIES.</p>
                    </div>
                    <div>
