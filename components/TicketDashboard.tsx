        ...
        <div className="bg-surface-dark p-6 rounded-lg shadow-xl">
            <div className="flex justify-between items-center mb-4 flex-wrap gap-4">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <input type="text" name="title" placeholder="Assunto..." value={filters.title as string} onChange={handleFilterChange} className="bg-gray-700 border border-gray-600 text-white rounded-md p-2 text-sm" />
                    ...
                    <tbody>
                        {tickets.length > 0 ? tickets.map((ticket) => (
                            <tr key={ticket.id} className="border-b border-gray-700 hover:bg-gray-800/50 cursor-pointer" onClick={() => onOpenActivities && onOpenActivities(ticket)}>
                                ...
                                <td className="px-6 py-4 font-bold text-white">{ticket.title}</td>
                                ...
                                <td className="px-6 py-4 text-center">
                                    <div className="flex justify-center gap-3">
                                        <button onClick={(e) => { e.stopPropagation(); if (onEdit) onEdit(ticket); }} className="text-blue-400 hover:text-blue-300"><EditIcon /></button>
                                    </div>
                                </td>
                            </tr>
            ...
            <Pagination currentPage={page} totalPages={Math.ceil(totalItems / pageSize)} onPageChange={onPageChange!} itemsPerPage={pageSize} onItemsPerPageChange={onPageSizeChange!} totalItems={totalItems} />
        </div>
    );
};
